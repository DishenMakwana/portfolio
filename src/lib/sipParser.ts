...
export function parseSipExcel(buffer: Buffer, filename: string): SipParsed {
  ...
  const sheetName = wb.SheetNames.find(n => n.toLowerCase().includes('sip')) || wb.SheetNames[0];
  ...
  const headerRowIdx = raw.findIndex(
    row => row && String(row[0] || '').trim().toUpperCase().includes('SR')
  );
  if (headerRowIdx === -1) throw new Error('Cannot find header row in SIP file');
  ...
  if (monthCols.length === 0) throw new Error('No monthly columns found in SIP file');
  ...
  for (let r = headerRowIdx + 1; r < raw.length; r++) {
    ...
    const srCell = row[0];
    // Skip "Total" row
    if (typeof srCell === 'string' && srCell.trim().toLowerCase() === 'total') break;
    ...
    if (nonZeroAmounts.length > 0) {
      ...
      nonZeroAmounts.forEach(v => { freq[v] = (freq[v] || 0) + 1; });
