...
export default function ZerodhaStocksTab({
  ...
  const stockBenchmarkTotals = useMemo(() => {
    return {
      benchmarkXirr: totals?.stocksBenchmarkXirr ?? 0,
      alpha: totals?.stocksAlpha ?? stockTotals?.avgAlpha ?? 0,
      portfolioXirr: totals?.stocksXirr ?? stockTotals?.avgXirr ?? 0,
    };
  }, [totals, stockTotals]);

  const stockBenchmarkDeltas = useMemo(() => {
    return {
      benchmarkXirr: metricDeltas?.stocksBenchmarkXirr ?? null,
      alpha: metricDeltas?.stocksAlpha ?? null,
      portfolioXirr: metricDeltas?.stocksXirr ?? null,
    };
  }, [metricDeltas]);
  ...
  return (
    <div className="space-y-6">
      <ZerodhaBenchmarkCards
        totals={stockBenchmarkTotals}
        metricDeltas={stockBenchmarkDeltas}
        title="Stocks XIRR"
      />
