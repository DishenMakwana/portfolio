...
import { uploadZerodhaCoinCsvAction } from "@/actions/zerodhaTransactionUpload";
import type {
  TransactionUploadModalProps,
  TransactionUploadResult,
} from "@/types/transactionUpload";

export default function ZerodhaTransactionUploadModal({
  isOpen,
  onClose,
}: TransactionUploadModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await uploadZerodhaCoinCsvAction(formData);
      setResult(res);
      ...
      setResult({
        success: false,
        message: "An unexpected error occurred during Coin CSV file upload.",
        totalProcessed: 0,
        insertedCount: 0,
        updatedCount: 0,
        skippedCount: 0,
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-extrabold text-slate-100">
                Upload Zerodha Coin Order History
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Import completed mutual fund purchase and sell orders from Coin
                (.csv) file
              </p>
            </div>
          </div>
            ...
            {selectedFile ? (
              ...
              <div className="space-y-2">
                ...
                <div className="text-xs font-semibold text-slate-300">
                  Drag and drop your Coin order history .csv file
                </div>
                <div className="text-[11px] text-slate-500">
                  Click to browse from your computer (e.g.
                  coin_order_history.csv)
                </div>
              </div>
            )}
          </div>
