...
export default function SipsClient({ mandates }: SipsClientProps) {
  ...
  const [uploadSuccess, setUploadSuccess] = useState<{ inserted: number; total: number } | null>(null);
  ...
  const filteredMandates = filterMember === 'all'
    ? mandates
    : mandates.filter(m => m.memberName === filterMember);

  const totalMonthly = mandates.filter(m => m.isActive).reduce((a, m) => a + m.monthlyAmount, 0);
  ...
  const monthCols = mandates.length > 0
    ? Object.keys(mandates[0].monthlyHistory)
    : [];
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Fixed monthly SIP schedule for all family members. Upload your &ldquo;My SIP&rsquo;s&rdquo; Excel export to sync.
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <label className="flex items-center gap-2 bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-slate-950 font-bold px-5 py-2.5 rounded-xl shadow-lg cursor-pointer transition text-sm">
              ...
              <input type="file" accept=".xlsx" onChange={handleUpload} className="hidden" disabled={isUploading} />
            </label>
          </div>
        </div>
      ...
      {mandates.length === 0 ? (
        ...
        <div className="bg-slate-900/40 border border-dashed border-slate-800 rounded-2xl p-16 text-center">
          ...
          <h2 className="text-xl font-bold text-slate-300 mb-2">No SIP data yet</h2>
          <p className="text-slate-500 text-sm">Upload your &ldquo;My SIP&rsquo;s&rdquo; Excel export from I Plus Financial Services to track your monthly mandates.</p>
        </div>
      ) : (
        <>
          {/* ── KPI SUMMARY CARDS ── */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { label: 'Total Monthly SIP', value: formatCurrency(totalMonthly), sub: 'Active mandates only', color: 'text-teal-400', icon: IndianRupee, bg: 'bg-teal-500/10', border: 'border-teal-500/20' },
              { label: 'Total Mandates', value: String(mandates.length), sub: `${activeSips} active`, color: 'text-slate-100', icon: Repeat2, bg: 'bg-slate-700/30', border: 'border-slate-700/50' },
              { label: 'Active SIPs', value: String(activeSips), sub: 'Running this month', color: 'text-emerald-400', icon: CheckCircle2, bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
              { label: 'Paused / Stopped', value: String(pausedSips), sub: 'Zero in latest month', color: 'text-amber-400', icon: AlertTriangle, bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
            ].map((card, i) => (
          ...
          <div className="space-y-4">
            {Object.entries(byMember).map(([memberName, sips]) => {
              const memberTotal = sips.filter(s => s.isActive).reduce((a, s) => a + s.monthlyAmount, 0);
              const isExpanded = expandedMember === null || expandedMember === memberName;

              return (
                <motion.div key={memberName}
                  initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                  className="bg-slate-900/70 backdrop-blur-md border border-slate-800/80 rounded-2xl overflow-hidden shadow-xl"
                >
                  {/* Member header */}
                  <button
                    onClick={() => setExpandedMember(expandedMember === memberName ? null : memberName)}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="font-bold text-slate-100 text-sm">{memberName}</div>
                        <div className="text-xs text-slate-500">
                          {sips.filter(s => s.isActive).length} active · {sips.filter(s => !s.isActive).length} paused
                        </div>
                      </div>
                    </div>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="bg-slate-950/50">
                                <th className="px-5 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap">Scheme</th>
                                <th className="px-5 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap">Folio</th>
                                <th className="px-5 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap">SIP Amount</th>
                                {monthCols.map(col => (
                                  <th key={col} className="px-3 py-2.5 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 whitespace-nowrap">{col}</th>
                                ))}
                                <th className="px-5 py-2.5 text-center text-[11px] font-semibold uppercase tracking-wider text-slate-500">Status</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800/40">
                              {sips.map((sip, i) => (
                                <tr key={sip.id} className={i % 2 === 0 ? '' : 'bg-slate-950/20'}>
                                  <td className="px-5 py-3 font-medium text-slate-200 max-w-xs">
                                    <div className="truncate">{sip.schemeName}</div>
                                  </td>
                                  <td className="px-5 py-3 font-mono text-xs text-slate-400">{sip.folioNo}</td>
                                  ...
                                  {monthCols.map(col => {
                                    ...
                                    return (
                                      <td key={col} className={`px-3 py-3 text-right text-xs font-semibold ${amt > 0 ? 'text-emerald-400' : 'text-slate-600'}`}>
                            ...
                            <tfoot>
                              <tr className="border-t border-slate-700/60 bg-slate-950/40">
                                <td colSpan={2} className="px-5 py-3 text-xs font-bold text-slate-400 uppercase">
                                ...
                                {monthCols.map(col => {
                                  const colTotal = sips.reduce((a, s) => a + (s.monthlyHistory[col] ?? 0), 0);
                                  return (
                                    <td key={col} className={`px-3 py-3 text-right text-xs font-bold ${colTotal > 0 ? 'text-teal-400' : 'text-slate-600'}`}>
          ...
          <div className="bg-slate-900/70 backdrop-blur-md border border-teal-500/20 rounded-2xl p-5 shadow-xl">
            <div className="flex flex-wrap justify-between items-center gap-4">
              <div className="text-sm font-bold text-slate-300">
                Family Grand Total — {activeSips} active SIPs across {members.length} investors
              </div>
              <div className="flex items-center gap-6">
                {monthCols.map(col => {
                  const colTotal = mandates.reduce((a, m) => a + (m.monthlyHistory[col] ?? 0), 0);
                  return (
                    <div key={col} className="text-center">
                      <div className="text-[11px] text-slate-500 font-medium">{col}</div>
                      <div className={`text-sm font-bold ${colTotal > 0 ? 'text-teal-400' : 'text-slate-600'}`}>
                ...
                <div className="text-center border-l border-slate-700 pl-6">
                  ...
                  <div className="text-xl font-extrabold text-teal-400">{formatCurrency(totalMonthly)}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {isUploading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-md">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl">
            ...
            <h3 className="text-xl font-bold text-slate-100">Parsing SIP file…</h3>
            <p className="text-slate-400 mt-2 text-sm">Reading monthly mandate amounts for all investors.</p>
          </div>
        </div>
      )}
    </div>
