...
const STATUS_META: Record<AutoMapResult['status'], {
  label: string; color: string; bg: string; border: string; icon: any;
}> = {
  mapped:          { label: 'Auto-Mapped',      color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/30', icon: CheckCircle2 },
  low_confidence:  { label: 'Low Confidence',   color: 'text-amber-400',   bg: 'bg-amber-500/10',   border: 'border-amber-500/30',   icon: HelpCircle },
  not_found:       { label: 'Not Found',         color: 'text-red-400',     bg: 'bg-red-500/10',     border: 'border-red-500/30',     icon: XCircle },
  already_mapped:  { label: 'Already Mapped',   color: 'text-teal-400',    bg: 'bg-teal-500/10',    border: 'border-teal-500/30',    icon: ShieldCheck },
  api_error:       { label: 'API Error',         color: 'text-slate-400',   bg: 'bg-slate-500/10',   border: 'border-slate-500/30',   icon: AlertTriangle },
};
...
export default function MappingTab({ allSchemes }: MappingTabProps) {
  ...
  const [autoMapResults, setAutoMapResults] = useState<AutoMapResult[] | null>(null);

  const [expandedResult, setExpandedResult] = useState<number | null>(null);
  const [autoMapFilter, setAutoMapFilter] = useState<'all' | 'mapped' | 'low_confidence' | 'not_found'>('all');

  // ── Search for the manual modal
  const handleApiSearch = async (query: string) => {
    setApiSearchQuery(query);
    if (query.trim().length < 3) { setApiSearchResults([]); return; }
    ...
    if (autoMapResults) {
      setAutoMapResults(prev =>
        prev
          ? prev.map(r =>
              r.schemeId === schemeId
                ? { ...r, status: code ? 'mapped' : 'not_found', schemeCode: code, confidence: code ? 100 : null }
                : r
            )
          : null
      );
  ...
  const filteredAutoResults = autoMapResults
    ? autoMapResults.filter(r => {
        ...
        if (autoMapFilter === 'mapped') return r.status === 'mapped' || r.status === 'already_mapped';
        if (autoMapFilter === 'low_confidence') return r.status === 'low_confidence';
        if (autoMapFilter === 'not_found') return r.status === 'not_found' || r.status === 'api_error';
        return true;
      })
    : null;
    ...
    >
      ...
      <div className="bg-slate-900/70 backdrop-blur-md border border-slate-800/80 rounded-2xl p-6 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          <div>
            <h3 className="text-xl font-bold text-slate-100">Scheme API Mappings</h3>
            <p className="text-slate-400 text-sm mt-1">
              Link each fund to its <span className="font-mono text-teal-400">api.mfapi.in</span> scheme code for live NAV, XIRR, and Alpha calculations.
            </p>
          </div>
          <button
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              {autoMapResults && (
                <div className="flex items-center gap-2 text-xs font-semibold">
                  <span className="text-emerald-400">{autoMapResults.filter(r => r.status === 'mapped').length} auto-saved</span>
                  <span className="text-slate-500">·</span>
                  <span className="text-amber-400">{autoMapResults.filter(r => r.status === 'low_confidence').length} need review</span>
                  <span className="text-slate-500">·</span>
                  <span className="text-red-400">{autoMapResults.filter(r => r.status === 'not_found' || r.status === 'api_error').length} not found</span>
                </div>
              )}
            </div>
            ...
            {isAutoMapping && (
              <div className="flex flex-col items-center justify-center py-10 gap-3">
                ...
                <p className="text-slate-400 text-sm">Querying api.mfapi.in for each scheme… this may take a moment.</p>
              </div>
            )}
            ...
            {autoMapResults && (
              <>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {(['all', 'mapped', 'low_confidence', 'not_found'] as const).map(f => (
                    <button key={f} onClick={() => setAutoMapFilter(f)}
                    ...
                    >
                      ...
                      {f === 'mapped' && `✅ Mapped (${autoMapResults.filter(r => r.status === 'mapped' || r.status === 'already_mapped').length})`}
                      {f === 'low_confidence' && `⚠️ Low Confidence (${autoMapResults.filter(r => r.status === 'low_confidence').length})`}
                      {f === 'not_found' && `❌ Not Found (${autoMapResults.filter(r => r.status === 'not_found' || r.status === 'api_error').length})`}
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                  {filteredAutoResults?.map(result => {
                    ...
                    return (
                      <div key={result.schemeId} className={`border ${meta.border} ${meta.bg} rounded-xl overflow-hidden`}>
                        <div
                          className="flex items-center justify-between p-3 cursor-pointer"
                          onClick={() => setExpandedResult(isExpanded ? null : result.schemeId)}
                        >
                          <div className="flex items-start gap-3 flex-1 min-w-0">
                            <Icon size={16} className={`${meta.color} shrink-0 mt-0.5`} />
                            <div className="flex-1 min-w-0">
                              <div className="font-semibold text-slate-200 text-sm truncate">{result.schemeName}</div>
                              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                                <span className={`text-xs font-bold ${meta.color}`}>{meta.label}</span>
                                {result.schemeCode && (
                                  <span className="text-xs font-mono text-slate-400">
                                    Code: <span className="text-teal-400 font-bold">{result.schemeCode}</span>
                                  </span>
                                )}
                                {result.confidence !== null && (
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className={`text-xs font-semibold ${result.confidence >= 75 ? 'text-emerald-400' : result.confidence >= 55 ? 'text-amber-400' : 'text-red-400'}`}>
                          ...
                          <div className="flex items-center gap-2 shrink-0 ml-2">
                            ...
                            <button
                              onClick={e => {
                                ...
                                const q = result.schemeName.replace(/Reg(?:ular)?/gi, '').replace(/\(G\)/g, '').trim().slice(0, 30);
                                setApiSearchQuery(q);
                                handleApiSearch(q);
                              }}
                          ...
                          {isExpanded && result.topMatches.length > 0 && (
                            ...
                            >
                              <div className="border-t border-slate-800/60 px-3 pb-3 pt-2 space-y-1">
                                <p className="text-xs text-slate-500 font-semibold mb-1.5">Top API matches — click to apply:</p>
                                {result.topMatches.map(match => (
                                  <div
                                    key={match.schemeCode}
                                    onClick={() => handleMapScheme(result.schemeId, String(match.schemeCode))}
                                  ...
                                  >
                                    <span className="text-xs text-slate-300">{match.schemeName}</span>
                                    <span className="text-xs text-teal-400 font-mono font-bold ml-3 shrink-0">{match.schemeCode}</span>
                                  </div>
                                ))}
                              </div>
                            </motion.div>
        ...
        <div className="space-y-3">
          {filteredSchemes.length === 0 ? (
            ...
            filteredSchemes.map((scheme) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-slate-200 text-sm">{scheme.name}</div>
                  <div className="text-xs text-slate-500 mt-1 flex flex-wrap items-center gap-2">
                    <span className="bg-slate-800 px-2 py-0.5 rounded text-slate-400">{scheme.category}</span>
                    {scheme.schemeCodeApi ? (
                      <span className="flex items-center gap-1 text-emerald-400 font-semibold">
                        <ShieldCheck size={12} />
                        Code: <span className="font-mono">{scheme.schemeCodeApi}</span>
                      </span>
                    ) : (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const q = scheme.name.replace(/Reg(?:ular)?/gi, '').replace(/\(G\)/g, '').trim().slice(0, 30);
                      setApiSearchQuery(q);
                      handleApiSearch(q);
                    }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 backdrop-blur-sm p-4">
          ...
          >
            <div className="p-5 border-b border-slate-800 flex justify-between items-start">
              <div>
                <h4 className="font-bold text-lg text-slate-100">Search Scheme Code</h4>
                <p className="text-xs text-slate-500 mt-0.5 max-w-md line-clamp-2">{mappingSchemeName}</p>
              </div>
              <button
            ...
            <div className="p-5 space-y-4">
              <div className="relative">
                ...
                {isSearchingApi && <Loader2 className="absolute right-3.5 top-3 text-teal-400 animate-spin" size={16} />}
              </div>
              ...
              <div className="h-64 overflow-y-auto bg-slate-950 rounded-xl border border-slate-800 p-2 space-y-1">
                {isSearchingApi ? (
                  ...
                  <div className="h-full flex items-center justify-center text-slate-500 text-sm">
                    {apiSearchQuery.trim().length < 3 ? 'Type at least 3 letters to search' : '❌ No results found'}
                  </div>
                ) : (
                  apiSearchResults.map((res: any) => (
                    <div
                      key={res.schemeCode}
                      onClick={() => handleMapScheme(mappingSchemeId, String(res.schemeCode))}
                    ...
                    >
                      <span className="font-medium text-slate-300 text-sm">{res.schemeName}</span>
                      <span className="text-teal-400 font-mono text-xs ml-4 font-bold shrink-0">#{res.schemeCode}</span>
                    </div>
                  ))
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-3 border-t border-slate-800">
                <span className="text-slate-400 text-xs shrink-0 font-medium">Enter code directly:</span>
                <div className="flex-1 flex gap-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      if (input?.value.trim()) handleMapScheme(mappingSchemeId, input.value.trim());
                    }}
